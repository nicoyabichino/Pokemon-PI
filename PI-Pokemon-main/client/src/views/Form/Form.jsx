...
export default function CreatePokemon(){
    ...
    function validate(input){
        ...
        if(parseInt(input.attack) < 1 || parseInt(input.attack) > 500){
            errors.attack = "The attack value must be between 1 and 500."
        }
        if(parseInt(input.defense) < 1 || parseInt(input.defense) > 200){
            errors.defense = "The defense value must be between 1 and 200."
        }
        if(parseInt(input.hp) < 1 || parseInt(input.hp) > 150){
            errors.hp = "The hp value must be between 1 and 150."
        }
        if(parseInt(input.speed) < 1 || parseInt(input.speed) > 200){
            errors.speed = "The speed value must be between 1 and 200."
        }
        if(parseInt(input.height) < 1 || parseInt(input.height) > 40){
            errors.height = "The height value must be between 1 and 40."
        }
        if(parseInt(input.weight) < 1 || parseInt(input.weight) > 999){
            errors.weight = "The weight value must be between 1 and 999."
        }
        return errors
    };
        ...
        <div className={style.all}>
            <form onSubmit={(e) => handleSubmit(e)}>
                ...
                <div className={style.divimg}>
                    ...
                    <img src={input.img? input.img : "https://38.media.tumblr.com/618ecd300cf043c09fe339d4a56239d3/tumblr_muhcuwrAmD1sq3bjlo1_500.gif"} alt="img" className={style.img}/>
                </div>
                </div>
